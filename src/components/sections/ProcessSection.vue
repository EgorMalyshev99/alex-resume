<template>
  <section id="process" class="py-16 lg:py-20" data-figma-node="1:290">
    ...
    <ol class="flex flex-col gap-4">
      ...
      >
        <div
          class="text-brand flex size-15 shrink-0 items-center justify-center rounded-2xl bg-neutral-100 font-sans
            text-xl font-semibold"
        >
          {{ String(idx + 1).padStart(2, '0') }}
        </div>
        <div class="flex flex-col gap-2">
          <h3 class="text-ink-dim font-sans text-lg font-semibold lg:text-xl">
            {{ step.title }}
          </h3>
...
const { tm } = useI18n()
const steps = computed<Step[]>(() => tm('process.steps'))
</script>
