          ...
          <div class="bg-surface shadow-card flex flex-col gap-3 rounded-xl p-6">
            <div class="flex flex-col gap-5 lg:flex-row">
              <div class="flex flex-1 flex-col gap-3">
                ...
                <p class="text-ink-dim text-lg leading-5.5">{{ $t('manager.overview') }}</p>
              </div>
              ...
              <div class="flex w-full flex-col gap-3 lg:w-75">
                ...
                <p class="text-ink-dim text-lg leading-5.5">{{ $t('manager.nda') }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-4 lg:flex-row">
              <div v-for="i in 3" :key="i" class="bg-surface shadow-card flex flex-1 flex-col gap-4 rounded-xl p-6">
                <p class="text-ink-dim text-xl leading-normal font-semibold">{{ $t(`manager.impact${i}Title`) }}</p>
                <p class="text-ink-dim text-base leading-5.5">{{ $t(`manager.impact${i}Body`) }}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="flex flex-col gap-16 lg:grid lg:grid-cols-[300px_minmax(0,1fr)] lg:gap-10">
            ...
            <div class="flex flex-col gap-16 lg:gap-30">
              <div id="goal" class="flex flex-col gap-3">
                <h2 class="text-ink-dim text-8 leading-normal font-bold">{{ $t('manager.goalTitle') }}</h2>
                <p class="text-ink-dim text-xl leading-7.5">{{ $t('manager.goalBody') }}</p>
              </div>

              <div id="my-role" class="flex flex-col gap-8">
                <div class="flex flex-col gap-3">
                  <h2 class="text-ink-dim text-8 leading-normal font-bold">{{ $t('manager.myRoleTitle') }}</h2>
                  <p class="text-ink-dim text-xl leading-7.5">{{ $t('manager.myRoleIntro') }}</p>
                </div>
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  ...
                  >
                    ...
                    <div class="flex flex-col gap-4">
                      ...
                      <p class="text-ink-dim text-lg leading-6">{{ $t(`manager.role${i}Body`) }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div id="process" class="flex flex-col gap-8">
                <div class="flex flex-col gap-3">
                  <h2 class="text-ink-dim text-8 leading-normal font-bold">{{ $t('manager.processTitle') }}</h2>
                  <p class="text-ink-dim text-xl leading-7.5">{{ $t('manager.processIntro') }}</p>
                </div>
                <div class="flex flex-col gap-6">
                  <div v-for="i in 4" :key="i" class="bg-surface shadow-card flex items-center gap-5 rounded-xl p-6">
                    ...
                    <div class="flex flex-col gap-3">
                      ...
                      <p class="text-ink-dim text-lg leading-6">{{ $t(`manager.process${i}Body`) }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div id="key-decisions" class="flex flex-col gap-8">
                <div class="flex flex-col gap-3">
                  <h2 class="text-ink-dim text-8 leading-normal font-bold">{{ $t('manager.decisionsTitle') }}</h2>
                  <p class="text-ink-dim text-xl leading-7.5">{{ $t('manager.decisionsIntro') }}</p>
                </div>
                <div class="flex flex-col gap-6">
                  <div v-for="i in 4" :key="i" class="bg-surface shadow-card flex gap-5 rounded-xl p-6">
                    <div
                    ...
                    <div class="flex flex-col gap-3">
                      ...
                      <p class="text-ink-dim text-lg leading-6">{{ $t(`manager.decision${i}Body`) }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div id="impact" class="flex flex-col gap-8">
                <div class="flex flex-col gap-3">
                  <h2 class="text-ink-dim text-8 leading-normal font-bold">{{ $t('manager.impactTitle') }}</h2>
                  <p class="text-ink-dim text-xl leading-7.5">{{ $t('manager.impactIntro') }}</p>
                </div>
                <ul class="flex flex-col gap-5">
                  <li v-for="i in 5" :key="i" class="flex items-start gap-3">
                    <span class="text-accent mt-1.75 shrink-0 text-base">→</span>
                    <p class="text-ink-dim text-xl leading-7.5">{{ $t(`manager.impactItem${i}`) }}</p>
                  </li>
                </ul>
              </div>
            </div>
